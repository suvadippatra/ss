...
async function previewOutputPage(toolId){
  ...
  try {
    ...
    const wrap = document.getElementById(toolId+'-prv-wrap');
    if (wrap) {
      wrap.innerHTML = '<div style="color:var(--red);padding:10px">⚠ Preview unavailable – pdf.js could not be loaded.<br><small>Add <code>pdf.min.js</code> and <code>pdf.worker.min.js</code> inside <code>./js/</code> folder.</small></div>';
  ...
  try {
    ...
    cvEl.width  = Math.round(vp.width);
    cvEl.height = Math.round(vp.height);
    await pg.render({canvasContext: cvEl.getContext('2d'), viewport: vp}).promise;
    wrap.style.display = 'block';
  } catch (e) {
    wrap.innerHTML = '<div style="color:var(--red);padding:10px">⚠ Preview rendering failed</div>';
    wrap.style.display = 'block';
    console.error(e);
  }
}
